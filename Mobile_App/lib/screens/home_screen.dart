...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void handleUrlCheck([String? url]) async {
    bool hasVerificationExceptionAlert = false;
    closeKeyboard();
    ...
    if (isValidUrl) {
      ...
      showAlert(
        ...
        alert: Alert(image: const AssetImage(CmhAssets.loadLogo)),
      );
      ...
      if (dataCertException == null) {
        ...
        hasVerificationExceptionAlert = true;
      ...
      if (context.mounted && !hasVerificationExceptionAlert) {
        hideAlert();
        if (dataCertException != null) {
          showVerificationExceptionAlert(dataCertException, context);
        } else {
  ...
  Future<bool> isCheckableUrl(String urlString) async {
    ...
    ).allMatches(url.host).isNotEmpty) {
      if (context.mounted) {
        await showAlert(
          barrierDismissible: true,
          context: context,
